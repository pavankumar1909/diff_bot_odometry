...
public:
    ...
    {
        client_ = create_client<diff_bot_msgs::srv::AddTwoInts>("add_two_ints");

        auto request = std::make_shared<diff_bot_msgs::srv::AddTwoInts::Request>();
        request->a = a;
        request->b = b;
...
private:
    rclcpp::Client<diff_bot_msgs::srv::AddTwoInts>::SharedPtr client_;

    void responseCallback(rclcpp::Client<diff_bot_msgs::srv::AddTwoInts>::SharedFuture future)
    {
        if(future.valid())
        {
