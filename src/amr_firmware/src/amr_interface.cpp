#include "amr_firmware/amr_interface.hpp"
...
CallbackReturn amrInterface::on_init(const hardware_interface::HardwareInfo &hardware_info)
{
  ...
  try
  {
    ...
    if(port_.empty())
    {
      RCLCPP_WARN(
        ...
        "No serial_port specified in hardware.Trying default /dev/ttyUSB0"
      );
      port_ ="/dev/ttyUSB0";
    }
  }
  ...
  RCLCPP_INFO(rclcpp::get_logger("amrInterface"), "The Serial Port provided is: %s", port_.c_str());
